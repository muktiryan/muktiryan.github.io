...
class DetailPostClass extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ id: id});
    console.log('######### '+ id);
    fetch('http://jsonplaceholder.typicode.com/posts?userId='+id).then(response => response.json()).then(data => this.setState({items: data, isLoading:false}))
  }
  render(){
    const {items, id, isLoading} = this.state
